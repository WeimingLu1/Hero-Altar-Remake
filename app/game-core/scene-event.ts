...
export function resolveSceneEvent(
  ...
  if (type === 4)
    return {
      lines: [String(originalText.start_fish || "你抛下了鱼线。")],
      tag: "fish",
    };
  if (type === 5)
    return {
  ...
  if (type === 8)
    return {
      lines: asLines(
        (originalText.boss_text as unknown[][])?.[id] || "杀气骤然逼近！",
      ).filter((_, i) => i % 2 === 1),
      tag: `boss:${id}`,
    };
  if (type === 9)
  ...
  if (type === 13) {
    const xy = (originalTasks.tan_map_xy as unknown[][])?.[id];
    if (xy)
      return {
  ...
  if (type === 16)
    return {
      lines: [String(originalText.welcome_home || "你走进桃花源小屋。")],
      tag: "enter-home",
    };
  void actor;
