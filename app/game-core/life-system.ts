...
export const emptySword = (): SwordData => ({
  forged: false,
  name: "",
  atk: 0,
  mid: 0,
...
// 四把自制武器的槽位：武器 ID 31–34 对应剑/刀/杖/鞭。
export const customWeaponSlot = (type: number) => 31 + type;
// 自制武器的词缀描述：攻击(atk)、中缀-闪避/命中(mid)、后缀-四维(suf)。
// 中缀/后缀的类型与数值由重铸时福缘(luck)与经验动态生成。
export function customSwordBonus(data: SwordData): string {
  const parts: string[] = [];
  if (data.atk) parts.push(`攻击+${data.atk}`);
  ...
  const suffixType = Math.floor(data.suf / 100),
...
export function customSwordDescription(data: SwordData, type: number): string {
  const typeName = swordTypes[type] || "兵";
  if (data.atk || data.mid || data.suf)
    return `经重铸淬炼的${typeName}器：${customSwordBonus(data)}（词缀品质随福缘而变）。`;
...
function swordList(actor: SceneActorState): SwordData[] {
  if (actor.swords?.length === 4) return actor.swords;
  actor.swords = [emptySword(), emptySword(), emptySword(), emptySword()];
  return actor.swords;
}
...
export function reforgeSword(
  ...
  const swords = swordList(actor),
    data = swords[type];
  if (!data || !data.forged) return { ok: false, text: "尚无此自制兵器。" };
  const needExp = (data.times + 1) * 100000,
  ...
  if (actor.weaponId === customWeaponSlot(type))
    return { ok: false, text: "请先卸下此自制兵器。" };
  actor.gold -= needGold;
