...
export type MapPage = { graphic: Record<string, unknown>; trigger: number; through: boolean; commands: RmxpCommand[] };
export type MapEvent = { id: number; name: string; x: number; y: number; pages: MapPage[] };
export type OriginalMap = { id:number; name:string; width:number; height:number; tileset_id:number; tiles:{x:number;y:number;z:number;data:number[]}; events:MapEvent[] };
export type WorldPosition = { mapId:number; x:number; y:number; direction:number };

const maps = (mapsJson as { maps: OriginalMap[] }).maps;
const mapIndex = new Map(maps.map(map => [map.id, map]));
const tilesets = (tilesetsJson as { data: Array<{id:number;passages:{data:number[]};priorities:{data:number[]}}> }).data;

export const originalMaps = maps;
export const originalStart: WorldPosition = { mapId: 4, x: 9, y: 7, direction: 2 };
...
export function tileAt(map: OriginalMap, x: number, y: number, layer: number) {
  return map.tiles.data[x + y * map.width + layer * map.width * map.height] || 0;
}

export function passable(map: OriginalMap, x: number, y: number, direction: 2|4|6|8) {
  if (x < 0 || y < 0 || x >= map.width || y >= map.height) return false;
  const tileset = tilesets.find(item => item.id === map.tileset_id) || tilesets[0];
  const passages = tileset.passages.data, priorities = tileset.priorities.data;
...
export function activePage(event: MapEvent) {
  ...
  return event.pages.find(page => page.commands.some(command => command.code !== 0)) || event.pages[0];
}
