...
// 每个 NPC 的专属闲聊池：同一角色每次对话都可能说出不同内容
const NPC_TALK: Record<string, string[]> = {
  laozhe: [
    "老朽见过太多人从这牌坊下走过，走出去的，都是带着答案的。",
    "你问老朽为何总晒太阳？因为晒着晒着，就把许多事看淡了。",
    "这镇子像一壶老茶，初喝苦，回甘慢。",
    "记住，功夫能救命，救不了心。"
  ],
  heiren: [
  ...
  axiu: [
    "山茶花开的时候，我总会想起娘。",
    "镇上的日子虽然清苦，但每天都有花卖，心里就不空。",
    "你要是喜欢，往后可以常来买花。",
    "花是给有心人看的，你懂么？"
  ],
  yuexia: [
    "红线这东西，看着轻，牵起来重。",
    "月下老人也不是什么都懂，我只懂怎么把两个人牵到一起。",
    "缘分到了，挡都挡不住。",
    "年轻人，心里有人了？"
  ],
  xiaoer: [
...
export function isQuestNpc(npcId: string, s?: GameState): boolean {
  const n = NPCS[npcId];
  if (!n) return false;
  if (n.questGiver || n.master) return true;
  if (npcId === "lengTieYi") return true;
  if (npcId === "laozhe" && s?.player.flags["xiaoyao"] && !s.player.flags["xiaoyaoDone"]) return true;
  return false;
}

export function randomNpcTalkDialog(npcId: string, s?: GameState): DialogNode[] {
  const npc = NPCS[npcId];
  const pool = NPC_TALK[npcId] || CHAT_FLAVOR;
  const used: string[] = [];
  const next = () => {
    let line = pickText(pool);
    for (let i = 0; i < 6 && used.includes(line); i++) line = pickText(pool);
    used.push(line);
    return line;
  };
  const name = npc?.name || "江湖客";
  return [
    {
      id: "r",
      speaker: name,
    ...
    { id: "bye", speaker: name, text: "「江湖路远，今日便聊到这里。」", opts: [] }
  ];
}

const ENCOUNTER_EVENTS = [
  (a: string, b: string) => `你远远看见${a}和${b}说了几句，${a}走时脸色有点怪。`,
  (a: string, b: string) => `${a}把一样东西塞给${b}，两人很快分开，像什么都没发生过。`,
  (a: string, b: string) => `${a}拦住${b}说了一通话，${b}听完低头想了很久。`,
  (a: string, b: string) => `${a}和${b}说着说着都笑了，路过的人也跟着笑了一声。`,
  (a: string, b: string) => `${a}在${b}耳边说了句什么，${b}摇头叹气，往反方向走了。`,
  (a: string, b: string) => `${a}递给${b}一只旧碗，${b}接过去，又往里面放了几枚铜板。`,
  (a: string, b: string) => `${a}和${b}并肩站了一会儿，谁也没说话，风倒是吹了很久。`,
  (a: string, b: string) => `${a}走后，${b}在原地站了许久，像是把某句话反复嚼了几遍。`
];

export function randomEncounterEvent(a: string, b: string): string {
...
const PLAYER_MOVE_TEXTS = [
  ...
  "你手腕一翻，招式未老已变，虚虚实实叫人难辨。",
  "你借着夜风之势纵身而起，衣袂猎猎，一掌凌空拍下。",
  "你足尖在碎石上一点，身形如游鱼般滑向侧翼。",
  "你双手抱圆，内力在经脉中一荡，招式竟比心意更快。",
  "你低喝一声，步法错开半尺，反手一击已递到对方面门。",
  "你身形一矮，避过对方视线，随即如绷簧般弹起。",
  "你凝神观气，见对方肩头微动，抢先一步封住来路。",
...
const ENEMY_MOVE_TEXTS = [
  "对方身形一晃，刀光自斜刺里卷来，快得只剩一道残影。",
  "对面一声低吼，拳风裹着沙尘扑面而至。",
  "对方脚下交错，连踏三步，掌势如潮水般压来。",
  "只听衣袂破风，对方已腾身而起，凌空一击直取天灵。",
  "对方双目一凝，兵刃在半空划出一道冷弧。",
  "对方足尖一点，身形陡然加快，抢进你怀里发难。",
  "对方气息一沉，双臂轮转，攻势一浪高过一浪。",
  "对方侧身避过你的视线，反手一招已无声递到。",
  "对方冷笑一声，身形忽左忽右，教你摸不准来路。",
  "对方脚下扬起尘土，借着扬尘之势掩杀而来。",
  "对方把兵器舞成一团光影，劈头盖脸压下来。",
  "对方眼神一厉，整个人的气势陡然拔高一截。",
  "对方踏出半弧，绕到你身侧，掌风贴着肋骨掠过。",
  "对方吐气开声，一招朴实无华，却带着千斤力道。",
...
export function randomPlayerMoveText(): string {
  return pickText(PLAYER_MOVE_TEXTS);
}

export function randomEnemyMoveText(enemyName: string): string {
  return `${enemyName}${pickText(ENEMY_MOVE_TEXTS)}`;
}
