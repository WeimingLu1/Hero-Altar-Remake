...
![英雄坛说：云游志标题界面](docs/screenshots/title-latest.png)

### 69张地图与城镇探索

![当前版本的完整游戏画面](docs/screenshots/world-latest.png)

网页版本保留原作69张地图及转场关系，同时重新组织道路、院落、围栏、植被和多格建筑。绿色门牌是可进入地点，玩家可以在平安镇、商业街、各大门派、洞府与险地之间探索；地图事件继续负责交易、任务、拜师、战斗和场景转移。

![商业街中的主角近身共享会话](docs/screenshots/player-conversation-latest.png)

商业街把药店、杂货店、当铺和其他可互动建筑连成街区。玩家靠近 NPC 并停下后会进入地图即时会话；主角参与的共享会话卡使用蓝色强调，卡片从主角头顶向上展开并始终位于最高绘制层。未参与谈话的同屏 NPC 仍可独立行动或自言自语。
...
玩家停在 NPC 身边时可以成为对话参与者；玩家移动后立即退出环境会话。主角参与时整组对话采用蓝色会话卡，完整显示每条台词，不限制两行；短内容贴近主角头顶，文字增加时保持底边不动并向上扩展，达到画面边缘后才钳制。
...
当前版本通过181项逻辑/视觉测试和3项服务端渲染测试，共184项，0失败。项目仍在积极开发中，接口、存档结构和美术资源可能继续调整。
