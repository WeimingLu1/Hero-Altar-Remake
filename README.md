...
- 主动交谈经过纯台词清洗；战斗小说化按本回合新增的原始日志逐条丰富续写，原文一条对应模型一段并全部展示，不再用额外事实校验重构或丢弃模型正文。段落颜色始终取自引擎标注，即使模型漏写标签也会按真实攻守方分色；只有请求真正失败时才回退确定性原作战报，战斗结算始终由引擎独立控制。
...
当前版本通过350项逻辑/视觉单元测试和3项服务端渲染测试，0失败；完整交互流程另在Chromium、Firefox、WebKit共执行30项E2E。项目同时通过类型检查、ESLint、核心源码覆盖率、依赖审计、生产构建与体积预算。生产地址为[首页](https://rmxp-hero-web.weminl.chatgpt.site)与[完整游戏](https://rmxp-hero-web.weminl.chatgpt.site/original)。项目仍在积极开发中，接口、存档结构和美术资源可能继续调整。
