...
> 当前生产版本为 Sites v113（2026-08-28），已经具备完整地图探索、原作任务与战斗、人物成长、v4 本地存档、NPC 动态活动、底部双立绘 LLM 主动交谈及三类生成奇遇。门派掌门与大雪山弟子使用分层级门派服装。NPC 长期记忆、自主目标和长期规划仍在未来路线图中。
...
| NPC 一对一环境会话 | 主角一对一环境会话 |
| --- | --- |
| ![NPC 一对一环境会话](docs/screenshots/npc-dialogue.jpg) | ![主角高亮对话](docs/screenshots/player-dialogue.jpg) |
...
- NPC自言自语、动作、一对一会话，以及玩家近身后与唯一一名 NPC 自动交谈。
- 对话参与者使用统一事实档案：年龄、性别、门派、师承、外貌、武境阶梯和兵刃均作为不可改写信息提供给模型。
- 统一LLM队列：玩家对话优先，其次为NPC近身对话、自言自语和动作；内部固定两条生成通道，纯NPC演出最多占一条。
- 严格3×3近身听觉圈：玩家和NPC只有近身才能对话，同屏但未近身只能独演。
- 一对一会话严格轮流发言；上一句停留结束并消失后，才允许另一人生成和显示下一句，同一会话永远只有一个可见气泡。
- 环境交谈按“陈述开场→补充细节→表达态度→偶尔追问”的话轮节奏生成；问句后下一人必须直接回答，后处理会移除陈述话轮末尾多余的追问，避免每句都以问号收尾。
- NPC与主角的会话台词优先显示在当前发言者头顶；冲突时依次尝试左右、脚下和邻近空位，并避开屏幕人物与地图标题。主角气泡使用亮蓝色粗体并最后绘制。
...
一对一交谈使用严格轮次锁，按“甲一句 → 气泡停留并消失 → 乙一句”的顺序显示；气泡以说话人头顶为首选，冲突时按侧边、脚下、邻近空位稳定避让。每次发言完整换行、不截断；自言自语和环境动作使用同一布局规则。
...
- 玩家进入同样的近身范围后，只与最近且事件ID稳定优先的一名NPC建立会话；NPC或玩家都可能自然开场。
...
标题页的“模型设置”可以修改服务地址、模型名和请求超时；打开设置本身不会访问模型，只有点击“仅检测连接”或“保存并检测”才发起健康检测。环境演出使用固定的玩家优先限流，不再提供手动并发设置。配置只保存在当前浏览器。
...
当前版本通过403项逻辑/视觉单元测试和3项服务端渲染测试，0失败；完整交互流程另在Chromium、Firefox、WebKit共执行30项E2E。项目同时通过类型检查、ESLint、核心源码覆盖率、依赖审计、生产构建与体积预算。生产地址为[首页](https://rmxp-hero-web.weminl.chatgpt.site)与[完整游戏](https://rmxp-hero-web.weminl.chatgpt.site/original)。项目仍在积极开发中，接口、存档结构和美术资源可能继续调整。
