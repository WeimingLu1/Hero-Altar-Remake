...
- 主动交谈经过纯台词清洗，战报必须通过引擎攻防段落顺序校验；不合规输出自动使用确定性战报，不会改写结算。
...
当前版本通过348项逻辑/视觉单元测试和3项服务端渲染测试，0失败；完整交互流程另在Chromium、Firefox、WebKit共执行30项E2E。项目同时通过类型检查、ESLint、核心源码覆盖率、依赖审计、生产构建与体积预算。生产地址为[首页](https://rmxp-hero-web.weminl.chatgpt.site)与[完整游戏](https://rmxp-hero-web.weminl.chatgpt.site/original)。项目仍在积极开发中，接口、存档结构和美术资源可能继续调整。
