...
- 可攻略：阿绣（女）、唐晚词（女）、李振威（男）、商剑鸣（男）；所有成年 NPC 也可谈心、亲近。
- 好感：谈心 +3，送礼按角色喜好，上限 100。
- 亲密：玩家与对象均须 ≥16 岁、好感 ≥60（已婚 40），无每日次数限制；演出为含蓄的“红烛小楼”淡出动画与诗意文字。结婚须 ≥18 岁（月下老人牵线，`marry` action）。
- **内容边界（硬约束）**：不得加入露骨性描写、不得加入强奸/性侵犯内容；恶行线“偷香”只是潜入取走绣帕（非性行为），三次得“采花大盗”名号。
...
- 天气：晴/雨/雪/雾/风（雨 10% 升级雷雨），随机变化，切换时粒子 1 秒淡入淡出；雨落地溅射、雪斜落、雾分前后两层、风按区域主题吹不同粒子；HUD 天气格带图标（☀️🌧️❄️🌫️💨⛈️）。
...
- **传闻体系**：统一出口 story.ts 的 `randomRumor(s?)`——常识 12 条 + 区域 28 条（14 区域各 2 条）+ 状态感知 10 组（周三已除/冷铁衣已除/云中鹤已除/石板 ≥1/≥3/善恶 ≤-20/≥60/采花大盗/已婚/主线完成），不传 state 只出常识池；闲聊气泡与开场志都传 state。app.ts 旧 randomRumorText 已删除。
- 地图收集物：金银光点/药草随机刷新，拾取提示，40 秒后易位重刷。
- NPC 头顶随机闲聊气泡（randomRumor(s)，生活演出期间暂停）；区域首次进入有“开场志”文案。

## 作弊器（F8）

- 改四项天赋、银两、潜能、经验、善恶、内力强度、年龄、任意武功等级、添加物品、瞬移、完成任务。
- 全武功 100 级、锁血无敌、经典黑白模式（`YOBDC` 创建名可解锁并送资源）。
...
- 对话里通用按钮：切磋武艺、查看状态、送礼/谈心/亲近（成人向）、偷香（恶行）
